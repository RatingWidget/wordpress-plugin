...
?>
<p>
	<input type="hidden" name="rw_comment_meta_box_nonce" value="<?php echo wp_create_nonce( basename( WP_RW__PLUGIN_FILE_FULL ) ); ?>" />
	<table class="rw-rating-table rw-comment-admin-rating rw-left">
		...
		<tr>
		<td>
			<div><nobr><?php echo ( isset( $criterion['label']) && ! empty( $criterion['label'] ) ) ? $criterion['label'] : ''; ?></nobr></div>
			<div class="rw-ui-container rw-class-<?php echo $criterion_rclass ?>" <?php echo $multi_criterion ? "data-uarid=\"$urid_summary\"" : ''; ?> <?php echo ( $multi_criterion || $default_hide_recommendations ) ? ' data-hide-recommendations="true" ' : ''; ?> data-urid="<?php echo $urid; ?>" data-sync="false"></div>
			<p></p>
		</td>
		</tr>
		<?php
